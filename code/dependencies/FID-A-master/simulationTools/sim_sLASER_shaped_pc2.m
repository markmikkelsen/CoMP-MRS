...
phArray=linspace(0,180-(180/pcSteps),pcSteps);
...
%for k=1:pcSteps                                                 %Loop through all phase cycle steps for first AFP pulse.  
    ...
    d=sim_shapedRF(d,H,RFX,tp,flipAngle,0,dx,Gx); %1st shaped 180 degree adiabatic refocusing pulse along X gradient
%    d_pc=sim_dAdd(d_pc,d_temp);                                 %Add the density matrices from each phase cycle step
%end
%d=sim_dDiv(d_pc,pcSteps);                                       %Divide by the number of steps

d=sim_evolve(d,H,tau2/1000);                                    %Evolve by tau2

d_pc = cell(size(d));                                           %Create empty cell for phase cycling;
for k=1:pcSteps                                                 %Loop through all phase cycle steps for second AFP pulse. 
...
%d_pc = cell(size(d));                                           %Create empty cell for phase cycling;
%for k=1:pcSteps                                                 %Loop through all phase cycle steps for third AFP pulse. 
    ...
    d=sim_shapedRF(d,H,RFY,tp,flipAngle,phArray(k),dy,Gy); %3rd shaped 180 degree adiabatic refocusing pulse along Y gradient
%    d_pc=sim_dAdd(d_pc,d_temp);                                 %Add the density matrices from each phase cycle step
%end
%d=sim_dDiv(d_pc,pcSteps);                                       %Divide by the number of steps
...
d_pc = cell(size(d));                                           %Create empty cell for phase cycling;
for k=1:pcSteps                                                 %Loop through all phase cycle steps for fourth AFP pulse. 
