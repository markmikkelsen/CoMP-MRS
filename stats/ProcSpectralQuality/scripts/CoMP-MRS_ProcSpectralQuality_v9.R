...
DATA_clean <- mad_outlier_removal(
  ...
  strategy  = "multilevel",  # options: "global", "group", "multilevel"
  threshold = 2.5
)
...
make_violin_plot <- function(data, x_var, y_var, y_label, file_name) {
...
for (y in y_specs) {
  for (x in x_vars) {
    ...
    all_boxplots[[key]] <- make_violin_plot(
...
# M.SNRLWrationorm.0.2 <- lmer(SNR_LW_Ratio_norm ~ (1 | MRvendor), data = DATA, REML = FALSE, control = optimToUse)
# M.SNRLWrationorm.0.3 <- lmer(SNR_LW_Ratio_norm ~ (1 | SiteID), data = DATA, REML = FALSE, control = optimToUse)
# M.SNRLWrationorm.0.4 <- lmer(SNR_LW_Ratio_norm ~ (1 | MRvendor) + (1 | SiteID), data = DATA, REML = FALSE, control = optimToUse)
M.SNRLWrationorm.0.full <- lme4::lmer(SNR_LW_Ratio_norm ~ (1 | MRvendor) + (1 | SiteID) + (1 | DP),
                                  data = DATA, REML = FALSE, control = optimToUse)
M.SNRLWrationorm.1.0 <- lme4::lmer(SNR_LW_Ratio_norm ~ (1 | DP) + (1 | MRbrainregion),
                             data = DATA, REML = FALSE, control = optimToUse)
# M.SNRLWrationorm.2.0 <- lmer(SNR_LW_Ratio_norm ~ (1 | MRvendor) + (1 | SiteID) + (1 | DP) +
#                                  (1 | AnimalSpecies) + (1 | MRbrainregion) , data = DATA, REML = FALSE, control = optimToUse)
# M.SNRLWrationorm.3.0 <- lmer(SNR_LW_Ratio_norm ~ (1 | MRvendor) + (1 | SiteID) + (1 | DP) +
...
M.SNRLWrationorm.4.0 <- lme4::lmer(SNR_LW_Ratio_norm ~ MRfield + (1 | DP),
                               data = DATA, REML = FALSE, control = optimToUse)
# M.SNRLWrationorm.5.0 <- lmer(SNR_LW_Ratio_norm ~ MRfield + AnimalAge + (1 | MRvendor) + (1 | SiteID) + (1 | DP) +
#                                  (1 | AnimalSpecies) + (1 | MRbrainregion) + (1 | MRsequence) , data = DATA, REML = FALSE, control = optimToUse)
# M.SNRLWrationorm.6.0 <- lmer(SNR_LW_Ratio_norm ~ MRfield + AnimalAge + AnimalSex + (1 | MRvendor) + (1 | SiteID) + (1 | DP) +
#                                  (1 | AnimalSpecies) + (1 | MRbrainregion) + (1 | MRsequence) , data = DATA, REML = FALSE, control = optimToUse)

### Model diagnostics ---------------------------------------------------------

model <- M.SNRLWrationorm.4.0
...
vc <- as.data.frame(VarCorr(M.SNRLWrationorm.0.1))[4]
vpc.M.0.1 <- round(vc/sum(vc)*100,1)
dimnames(vpc.M.0.1)[[1]] <- c("DP", "Residual")

# print(VarCorr(M.SNRLWrationorm.0.5), comp = c("Variance", "Std.Dev."))
# vc <- as.data.frame(VarCorr(M.SNRLWrationorm.0.5))[4]
...
LRT.1 <- anova(M.SNRLWrationorm.0.1, M.SNRLWrationorm.4.0)
...
clusterExport(clus, c("nloptr", "defaultControl", "nloptFun",
                      "DATA", "M.SNRLWrationorm.4.0", "M.SNRLWrationorm.0.1"))

# Bootstrap likelihood ratio tests
PB_LRT.1 <- PBmodcomp(M.SNRLWrationorm.4.0, M.SNRLWrationorm.0.1, nsim = 2e3, cl = clus)
