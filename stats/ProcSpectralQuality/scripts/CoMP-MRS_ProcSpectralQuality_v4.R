...
# Optional categorical version of MRaverages for pie charting -----------------

DATA <- DATA %>%
  mutate(
    MRaverages_cat = cut(
      MRaverages,
      breaks = c(-Inf, 64, 128, 256, 512, Inf),
      labels = c("<=64", "65-128", "129-256", "257-512", ">512"),
      right = TRUE
    )
  )
...
# Pie chart styling closer to the reference figure -----------------------------

# Custom color palette similar to the figure
pie_palette <- c(
  "#4C77C2",  # blue
  "#EA7E2D",  # orange
  "#A7A7A7",  # gray
...
make_pie_chart <- function(data, var, plot_title = NULL, file_name = NULL, output_dir) {
  ...
  plot_data <- data %>%
    ...
    dplyr::mutate(
      perc = n / sum(n),
      perc_label = scales::percent(perc, accuracy = 0.01),
      count_label = as.character(n),
      legend_label = paste0(as.character(!!var_sym), " (", n, "; ", perc_label, ")")
    ) %>%
  ...
  p <- ggplot(
    plot_data,
    aes(x = 1, y = perc, fill = legend_label)
  ) +
    geom_col(
      color = "#5A5A5A",
      linewidth = 1.2,
...
pie_specs <- list(
  ...
  list(var = "MRaverages_cat", title = "Nr. of averages (#DPs)")
)
...
# 1) SNR_LW_Ratio_norm by MRvendor
p_vendor <- ggplot(DATA, aes(x = MRvendor, y = SNR_LW_Ratio_norm, fill = MRvendor)) +
  geom_boxplot(
    width = 0.65,
    alpha = 0.9,
    outlier.shape = 21,
  ...
  labs(
    title = "Normalized SNR/LW ratio by vendor",
    x = "Vendor",
...
ggsave(
  file.path(plots_dir, "boxplot_SNR_LW_Ratio_norm_by_MRvendor.png"),
  plot = p_vendor,
  width = 6,
  height = 5,
  units = "in",
...
# 2) SNR_LW_Ratio_norm by MRvendor, faceted by AnimalSpecies
p_vendor_species <- ggplot(DATA, aes(x = MRvendor, y = SNR_LW_Ratio_norm, fill = MRvendor)) +
  geom_boxplot(
    width = 0.65,
    alpha = 0.9,
    outlier.shape = 21,
  ...
  labs(
    title = "Normalized SNR/LW ratio by vendor and animal species",
    x = "Vendor",
    y = "Normalized SNR/LW ratio"
  ) +
  scale_fill_brewer(palette = "Set2") +
...
print(p_vendor_species)

ggsave(
  file.path(plots_dir, "boxplot_SNR_LW_Ratio_norm_by_MRvendor_AnimalSpecies.png"),
  plot = p_vendor_species,
  width = 8,
  height = 5,
  units = "in",
  dpi = 300
