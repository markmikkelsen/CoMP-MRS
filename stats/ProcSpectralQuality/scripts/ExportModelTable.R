...
ExportModelTable <- function(models,
  ...
  if (!is.null(vpcs) && length(vpcs) > 0) {
    ...
    rows <- lapply(all_grps, function(grp) {
      vals <- vapply(model_names, function(nm) {
        vpc_df <- vpcs[[nm]]
  ...
  notes <- c(
    ...
    "All models fit with REML. Dependent variable: normalized SNR/LW ratio.",
    "Intercept-only fixed structure; random-effect grouping factors vary across models.",
